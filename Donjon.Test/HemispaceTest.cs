...
public class HemispaceTest(ITestOutputHelper outputHelper) : HostedTestBase<HemispaceTest>(outputHelper)
{
    ...
    public void CoordConversion()
    {
        // Given
        Coord c = (3, 4);
        Realspace<Coord> rc = c;
        Assert.Equal(rc.Value, c);

        Hemispace<Coord> h = rc.ToHemi();
        Assert.Equal(h.Value, (Coord)(1, 2));
        
        // (int, int) decomp = rc;
        // (Realspace<int>, Realspace<int>) decomp1 = rc;
    ...
    [Theory(Skip = "Known reconstitution semantic failure")]
    ...
    [Fact(Skip = "Known reconstitution semantic failure")]
