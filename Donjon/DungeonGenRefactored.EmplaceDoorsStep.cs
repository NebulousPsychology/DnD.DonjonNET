...
namespace Donjon
{
    ...
    {
        /// <summary>
        /// place doors using open_rooms
        /// </summary>
        /// <param name="logger"></param>
        /// <param name="settings"></param>
        /// <param name="random"></param>
        public class EmplaceDoorsStep(
            ILogger<EmplaceDoorsStep> logger,
        ...
        : Pipeline.IPipelineOperation<IDungeon>
        {
            ...
            {
                using (logger.BeginScope<EmplaceDoorsStep>(this))
                {
                    try
                    {
                        logger.LogDebug("Placing doors using {}", nameof(open_rooms));
                        result = open_rooms(input);
                        return true;
                    }
                    catch (Exception any)
                    {
                        logger.LogError(any, "{name} failed", nameof(EmplaceDoorsStep));
                        result = null;
                        return false;
                    }
                }
            ...
            /// <returns></returns>
            IDungeon open_rooms(IDungeon dungeon)
            {
                /*
                //:: sub open_rooms {
                //::   my ($dungeon) = @_;
                ...
                */
                ...
                {
                    if (dungeon.room.Count == 0)
                        logger.LogWarning("zero rooms to open!?");
                    else
            ...
            /// <returns></returns>
            IDungeon open_room(IDungeon dungeon, IDungeonRoom room)
            {
                /*
                //:: sub open_room {
                //::   my ($dungeon,$room) = @_;
                ...
                */
                using (logger.BeginScope(nameof(open_room)))
                {
                    ...
                    for (int i = 0; i < n_opens; i++)
                    {
                        ...
                        if (OpenDoorCommand.TryCreate(random, sills_list, out var doorCmd))
                        {
                        ...
                        var SillIndex = random.Next(sills_list.Count());
                        ...
                        using (logger.BeginScope("OpenDoor"))
                        {
                            ...
                            {
                                Realspace<Coord> d = (Coord)sill.dir * x + (sill.sill_r, sill.sill_c);
                                dungeon.cell.Set(d, v => (v &= ~Cellbits.PERIMETER) | Cellbits.ENTRANCE);
                            }

                            var door_type = OpenDoorCommand.GetDoorType(random);
            ...
            int alloc_opens(IDungeon dungeon, IDungeonRoom room)
            {
                /*
                //:: sub alloc_opens {
                //::   my ($dungeon,$room) = @_;
            ...
            /// <param name="room"></param>
            /// <returns></returns>
            IEnumerable<Sill> door_sills(IDungeon dungeon, IDungeonRoom room)
            {
                /*
                //:: sub door_sills {
                //::   my ($dungeon,$room) = @_;
                ...
                */
                using (logger.BeginScope(nameof(door_sills)))
                {
                    ...
                    if (room.south <= settings.Value.Dungeon.n_rows - 3)// if south is inset 3 from map's bottom edge, there's enough room to exit south
                    {
                    ...
                    if (room.east <= settings.Value.Dungeon.n_cols - 3) // if east is inset 3 from map's right, there's enough room to exit east
                    {
                        for (int r = room.north; r <= room.south; r += 2)
                        {
            ...
            Sill? check_sill(Cellbits[,] cell, IDungeonRoom room, Realspace<int> sill_r, Realspace<int> sill_c, Cardinal dir)
            {
                ...
                */
                ...
                {
                    // take one step from sillPt in the opening direction
                    Realspace<Coord> doorPt = (sill_r, sill_c) + (Coord)dir;
                    //::      return unless ($door_cell & $PERIMETER);
                    if (cell.Get(doorPt).HasFlag(Cellbits.PERIMETER) is false) return null; // return because of not being a perimeter
                    //::      return if ($door_cell & $BLOCK_DOOR);
                    if (cell.Get(doorPt).HasFlag(Cellbits.BLOCK_DOOR) is true) return null; // return because of being blocked

                    // the first space outside the door cell (one further step from sillPt in the opening direction)
                    Realspace<Coord> outPt = doorPt + (Coord)dir;
                    Cellbits out_cell = cell.Get(outPt);
                    // cannot open into a block
                    if (out_cell.HasFlag(Cellbits.BLOCKED)) return null;

                    // cannot open into the same room served by the door
                    if (out_cell.TryGetRoomId(out int out_id) && out_id == room.id) return null;
                    ...
                    return new Sill()
                    {
                        ...
                        sill_c = sill_c,
                        dir = dir,
                        door_r = doorPt.Value.r,
                        door_c = doorPt.Value.c,
                        out_id = out_id,
                    };
                }
            }
    ...
    static class SillHelper
    {
        ...
        /// <param name="room"></param>
        /// <returns></returns>
        public static SillActionKind AttemptConnection(this Sill sill, IDungeon dungeon, IDungeonRoom room)
        {
    ...
    namespace Rooms.Commands
    {
        class OpenDoorCommand : MultipleTransactionBase<IDungeon>
        {
            ...
            /// 
            /// </summary>
            /// <see cref="DungeonGenRefactored.open_room(IDungeon, IDungeonRoom)"/>
            /// <param name="rng"></param>
            /// <param name="sills"></param>
            /// <param name="dun"></param>
            public OpenDoorCommand(Random rng, List<Sill> sills, IDungeon dun)
            {
                ...
                foreach (Coord c in Enumerable.Range(0, 4)
                    .Select(x => (sill.sill_r, sill.sill_c) + ((Coord)sill.dir * x))
                    )
                {
                    var storeme = new ReversibleSingleCellChangeCommand(
                        c.r, c.c,
                        startState: dun.cell.At(c),
                        endState: Cellbits.ENTRANCE | dun.cell.At(c) & ~Cellbits.PERIMETER);
                    this.Add(storeme);
                }


                (char? sign, string key, string type) = DungeonGenRefactored.EmplaceDoorsStep.Lookup_doorinfo(DoorType);
