...
public class SingleCellChangeCommand : ITransaction<IDungeon>
{
    ...
    protected Original.Cellbits After { get; init; }
    public SingleCellChangeCommand(int row, int col, Original.Cellbits endState)
    {
        Coordinate = (row, col);
        After = endState;
    }
...
{
    public ReversibleSingleCellChangeCommand(int row, int col, Original.Cellbits startState, Original.Cellbits endState)
    : base(row, col, endState)
    {
        Before = startState;
    }

    public ReversibleSingleCellChangeCommand(int row, int col, IDungeon startCondition, Original.Cellbits endState)
    : this(row, col, startCondition.cell[row, col], endState)
    {
    }

    protected Original.Cellbits Before { get; init; }
...
public class CellCallbackTransaction(Point coord, Func<Original.Cellbits, Original.Cellbits> func) : IReversibleTransaction<IDungeon>
{
    public Original.Cellbits? Before { get; private set; } = null;

    public void Execute(IDungeon d)
    {
        Before = d.cell[coord.Y, coord.X];
        d.cell[coord.Y, coord.X] = func(d.cell[coord.Y, coord.X]);
    }
    ...
    public void Undo(IDungeon d)
    {
        if (Before is Original.Cellbits b)
        {
            d.cell[coord.Y, coord.X] = b;
            Before = null;
        }
    }
}
...
class CellTransactionBuilder
{
    ...
    public CellTransactionBuilder Transform(Func<Original.Cellbits,Original.Cellbits> fn)
    {
        return this;
    }
